...
export default {
  ...
  methods: {
    deletInitData() {
      ...
      this.$refs["from-tree"].setCheckedNodes(this.self_to_data);
    ...
    addToAims() {
      ...
      // 传递信息给父组件
      this.$emit('listenChildAction',this.self_to_data);
  ...
  watch: {
    // self_to_data(val) {
    //   console.log('--------',val);
    //   this.$emit('listenChildAction',val);
    // },
    // 左侧 状态监测
    from_check_keys(val) {
      if (val.length > 0) {
...
select[data-v-656994a5] {
  background-color: #fff;
}
.transfer[data-v-656994a5] {
  position: relative;
  overflow: hidden;
}
.transfer-left[data-v-656994a5] {
  position: absolute;
  top: 0;
  left: 0;
}
.transfer-right[data-v-656994a5] {
  position: absolute;
  top: 0;
  left: 300px;
}
.transfer-main[data-v-656994a5] {
  padding: 10px;
  height: calc(100% - 41px);
...
.transfer-center[data-v-656994a5] {
  position: absolute;
  top: 50%;
  left: 220px;
  width: 60px;
...
.transfer-center-item[data-v-656994a5] {
  padding: 10px;
  overflow: hidden;
}
.transfer-title[data-v-656994a5] {
  border-bottom: 1px solid #ebeef5;
  padding: 0 15px;
  height: 40px;
  line-height: 40px;
...
.transfer-title .el-checkbox[data-v-656994a5] {
  margin-right: 10px;
}
...
// WEBPACK FOOTER //
// src/components/transfer-extend.vue
