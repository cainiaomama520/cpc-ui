	...
	<div class="adCreativity">
		<div class="mediaMidden">
			...
			<div class="itemDiv">
				...
				<el-button  @click="export2Excel()" type="primary" size="small">链接下载</el-button>
				<el-table :data="tableListData" :summary-method='getSummaries' show-summary style="width: 100%" v-loading="tableLoading">
					<template v-for="(col ,ro) in tableHeaderName">
						<el-table-column v-if='col.value' :label='col.name' :fixed="0 == ro ? true:false" width="140" :prop="ro == 0 ? 'updateDate' : col.label" sortable>
				...
				v.$api.get(v.allurlsList.creatadver, params, function(r) {
					...
					if(r.content != null && r.content.length > 0) {
						...
						for(var i = 0; i < r.content.length; i++) {
							...
							dic.advtracking = r.content[i].trackLink.name;
							dic.linkdetail = JSON.parse(r.content[i].content).clk;
					...
					for (let [i, creativeId] of (_this.pageCreativeIds||[]).entries()) {
						if ((_this.pageCreativeStats[''+creativeId]|| 0 == _this.pageCreativeStats[''+creativeId]) && _this.tableListData[_this.pageCreativeStats[''+creativeId]]){
							...
							tmp.cliCondition= '点击数：' + (stats[''+creativeId] && stats[''+creativeId].clkValidCount || 0) + '\n' +
									'点击率：' + (tmp.imp/(stats[''+creativeId] && stats[''+creativeId].clkValidCount||1)).toFixed(2) + '\n' +
