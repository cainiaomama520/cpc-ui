<template lang="html">
	<div class="adCreativity">
		<div class="mediaMidden">
						...
						<el-form>
							<el-form-item label="广告位:">
								<el-select v-model="RTBId" filterable placeholder="" style="width:200px">
									<el-option key="" label="全部" value=""></el-option>
									<el-option v-for="(item,index) in RTBList" :key="item.id" :label="item.name" :value="item.id">
									</el-option>
								</el-select>
							</el-form-item>
						</el-form>
			...
			<div class="itemDiv">
				<el-button v-if="isMediaBuy && (tableListData.length != 0)" @click="export2Excel()" type="primary" size="small">链接下载</el-button>
				<el-table :data="tableListData" style="width: 100%">
					<el-table-column type="expand" fixed="left">
						<template slot-scope="props">
							<el-form label-position="left"  inline class="demo-table-expand" style="width:450px;float:left; margin-top:1px;margin-left:150px">
	      						<el-form-item label="广告主：">
							...
							<div style="width:246px;margin-left: 550px" >
								...
								<div v-else-if="props.row.imgObj.name == '微博 品牌大card' ||props.row.imgObj.name == '微博 普通博文' || props.row.imgObj.name == '微博 九宫格样式'">
									<div class="bigCardImgDiv" >
										<div class="contentDiv">
											...
											<img v-if="props.row.imgObj.name != '微博 九宫格样式' && props.row.imgObj.urls.length > 0" class="bigCardImg" :src="props.row.imgObj.urls[0].url" alt="">
											...
											<div v-if="props.row.imgObj.name == '微博 品牌大card'" class="cardMessageDiv">
					...
					<template v-for="(col ,ro) in tableHeaderName">
						<el-table-column v-if='col.value' :label='col.name' :fixed="0 == ro ? true:false" width="140" :prop='col.label' sortable>
							<template slot-scope="scope">
								...
								<div v-else-if="col.label == 'tfStatus'">
									<div>
										<el-switch style="display: block;height:100%;margin-top:10px" v-model="scope.row.tfStatus" active-color="#13ce66" inactive-color="#ff4949" active-text="开启" inactive-text="关闭" @change='statusChangeAction(scope.row)'>
										</el-switch>
									</div>
								</div>
								<div v-else>
					...
					<el-table-column  fixed="right" label="操作" width="200">
						<template slot-scope="scope">
							<el-button v-if="scope.row.total.adverCheckStatus !='' && scope.row.total.mediaType == 'RTB'" @click="adversongshenAction(scope.row)" type="text" size="small">广告主送审</el-button>
							<el-button v-if="scope.row.total.mediaType == 'RTB'" @click="songshenAction(scope.row)" type="text" size="small">创意送审</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			export2Excel() {
				// console.log('this.', this.nativeCreatives, this.adGroupList);
				require.ensure([], () => {
					const {
						export_json_to_excel
					} = require('../../../vendor/Export2Excel');
					const tHeader = ['链接ID', '链接名称', '外采媒体', '客户', '产品', '链接详情'];　　　　　　
					const filterVal = ['link_id', 'link_name', 'mediabuy', 'advertiser', 'product', 'link_detail'];
					let list = [];
					for(let [i, creinfo] of this.nativeCreatives.entries()) {
						list.push({
							link_id: creinfo.id || 0,
							link_name: creinfo.name || '',
							mediabuy: creinfo.mediaBuyResource && creinfo.mediaBuyResource.name || '',
							advertiser: creinfo.advertiser && creinfo.advertiser.name || '',
							product: creinfo.product && creinfo.product.name || '',
							link_detail: JSON.parse(creinfo.content || "{}").clk || ''
						})
					}　　　　　　
					const data = this.formatJson(filterVal, list);　　　　　　
					export_json_to_excel(tHeader, data, this.adGroupList[0] && this.adGroupList[0].name || '投放链接');
				});
			},
					...
					if(r.content != null && r.content.length > 0) {
						...
						for(var i = 0; i < r.content.length; i++) {
							...
							dic.cliCondition = '点击数：' + (r.content[i] && r.content[i].reportDTO && r.content[i].reportDTO.clkValidCount || 0) + '\n' +
								...
								'点击成本：' + ((dic.totalCvs / (r.content[i] && r.content[i].reportDTO && r.content[i].reportDTO.clkValidCount || 1)).toFixed(2));

							var status;
							if(r.content[i].checkStatus == undefined) {
								status = '未审核';
							} else {
								if(r.content[i].checkStatus.checkStatus == 'init') {
									status = '未审核';
								} else if(r.content[i].checkStatus.checkStatus == 'failed') {
									status = '审核失败：'+r.content[i].checkStatus.msg;
								} else if(r.content[i].checkStatus.checkStatus == 'pending') {
									status = '审核中';
								} else if(r.content[i].checkStatus.checkStatus == 'pass') {
									status = '审核通过';
								}
							}
							var checkSta;
